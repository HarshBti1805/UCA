...
char *encode(const unsigned char *data, int length) {
  ...
  while (i < length) {
    ...
    unsigned int b2 = (ipublic class Base64 { < length) ? data[i++] : 0; // Third Byte

      unsigned int block =
          (b0 << 16) | (b1 << 8) | b2; // Combining the Three Bytes

      // >>> represents the unsigned right shift AND 03xF masks the last 6 bits.
      // Since we need 4 chars we do it 4 times taking 6 bits out of the total
      // 24 bits every time. These 6 bits represent the index for the ALPHABET
      // Array. So in the result append ALPHABET[index]
  ...
  unsigned char *decode(const char *base64, int *out_len) {
    // Handling Padding. Base64 strings can end with = or == to make the length
    // a multiple of 4.
    ...
    while (i < len) {
      ...
      int block = (c0 << 18) | (c1 << 12) | (c2 << 6) |
                  c3; // Recombine into 24 bit block
