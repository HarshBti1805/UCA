public class SearchMatrix {
     ...
     */
    public boolean dfs(int r, int c, int[][] matrix, int target) {
        ...
        if (target < matrix[r][c]) {
            return dfs(r, c - 1, matrix, target); // move LEFT
        } else {
            return dfs(r + 1, c, matrix, target); // move DOWN
        }
    }
    public boolean searchMatrix(int[][] matrix, int target) {
        ...
        return dfs(startRow, startCol, matrix, target);
    }
