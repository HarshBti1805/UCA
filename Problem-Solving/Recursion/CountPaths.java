 ...
 */
 ...
 public class CountPaths {
  ...
  public int dfs(int x, int y, int m, int n){
      ...
      int up = dfs(x - 1, y, m, n);
      int left = dfs(x, y - 1, m, n);

      return up + left;
  }
  public int memoization(int x, int y, int m, int n, int[][] dp){
      if(x == 0 && y == 0) return 1;
      if(x < 0 || y < 0 || x >= m || y >= n) return 0;

      if(dp[x][y] != -1) return dp[x][y];

      int up = memoization(x - 1,y , m, n, dp);
      int left = memoization(x, y - 1 , m, n , dp);
  ...
  public static void main(String[] args) {
    ...
    assert cp.countPaths(3, 3, 0, 0) == 1 : "Test case 3 failed";
