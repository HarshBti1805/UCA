...
public class HumanPyramid {
  ...
  public double totalWeightSupported(List<List<Double>> pyramid, int row, int col) {
        // Base case: top person has no one above
        if (row == 0 && col == 0) {
            return pyramid.get(0).get(0);
        }

        // Own weight
        double weight = pyramid.get(row).get(col);

        // If person has someone above-left
        if (col > 0) {
            weight += 0.5 * totalWeightSupported(pyramid, row - 1, col - 1);
        }

        // If person has someone above-right
        if (col < row) {
            weight += 0.5 * totalWeightSupported(pyramid, row - 1, col);
        }
